...
Dari sisi arsitektur pipeline, tahap pra-pemrosesan pada CPU memanfaatkan multithreading — menggunakan pustaka seperti Rust Rayon, OpenMP, atau C++ Threads — untuk menangani proses flattening kurva, kalkulasi winding number, tiling geometri, dan penyusunan struktur data spasial secara paralel. Tahap rendering pada GPU dibatasi secara ketat pada penggunaan rasterization pipeline tradisional yang terdiri dari vertex shader dan fragment shader. Penggunaan compute shader, geometry shader, tessellation shader, maupun fitur spesifik perangkat keras tidak akan diterapkan demi menjaga kompatibilitas lintas platform.

Dari sisi lingkungan pengembangan, purwarupa (prototype) dibangun menggunakan bahasa pemrograman Rust dan API grafis lintas platform yang tidak mewajibkan fitur compute, yakni OpenGL ES 3.0 atau WebGL 2.0. Pilihan ini memastikan bahwa hasil penelitian dapat direplikasi dan dijalankan pada perangkat yang tidak mendukung Vulkan maupun WebGPU.

Dari sisi pengujian dan evaluasi, pengukuran performa difokuskan pada dua metrik utama: waktu rendering per bingkai (frame time) sebagai indikator kecepatan sistem, dan skalabilitas sistem terhadap penambahan jumlah core CPU selama fase pra-pemrosesan. Sebelum pengujian performa dilaksanakan, validasi kebenaran output (correctness validation) dilakukan terlebih dahulu dengan membandingkan hasil rendering purwarupa secara visual terhadap output renderer referensi pada sekumpulan berkas SVG uji yang telah ditentukan.
...
Tujuan utama penelitian ini adalah mendemonstrasikan kelayakan rendering vektor berkinerja tinggi pada lingkungan grafis yang tidak menyediakan dukungan compute shader, dengan cara merancang dan mengevaluasi arsitektur pipeline hibrida CPU-GPU yang mempertahankan sifat paralelisme di kedua sisi pipeline. Secara lebih rinci, penelitian ini bertujuan untuk mencapai hal-hal berikut:

1. Merancang arsitektur pipeline rendering vektor hibrida yang mengeliminasi ketergantungan pada compute shader tanpa mengorbankan sifat paralelisme, dengan pembagian beban kerja yang jelas antara CPU sebagai unit pra-pemrosesan paralel dan GPU sebagai unit rasterisasi.
2. Mengevaluasi efektivitas teknik paralelisasi masif pada CPU dalam menangani kompleksitas jalur vektor sebelum data dikirimkan ke GPU untuk tahap rendering.
3. Mengukur dan menganalisis performa purwarupa yang dihasilkan dalam hal kecepatan rendering (frame time) dan skalabilitas terhadap penambahan core CPU, serta membandingkannya dengan renderer referensi.
...
3. **Studi Dokumentasi Perangkat Lunak:** Dilakukan analisis terhadap dokumentasi API grafis OpenGL ES dan WebGL serta kemampuan multithreading bahasa pemrograman Rust untuk memahami batasan teknis yang berlaku dalam lingkungan tanpa compute shader.
...
2. **Perancangan Arsitektur Pipeline:** Merancang pembagian beban kerja hibrida secara rinci, di mana tahap pra-pemrosesan — meliputi flattening kurva Bézier menjadi segmen garis, kalkulasi winding number, tiling geometri, dan penyusunan struktur data spasial — dirancang untuk dieksekusi secara paralel di CPU. Tahap rendering berbasis implicit function atau pendekatan sejenis dirancang untuk dieksekusi di GPU melalui vertex shader dan fragment shader konvensional.
3. **Implementasi Purwarupa:** Membangun purwarupa menggunakan bahasa pemrograman Rust dengan pustaka Rayon untuk paralelisasi CPU, dan OpenGL ES sebagai antarmuka rasterization pipeline GPU. Implementasi mencakup seluruh alur kerja dari pembacaan berkas SVG hingga output piksel akhir.
4. **Pengujian dan Evaluasi:** Tahap ini dibagi menjadi dua sub-tahap: pertama, validasi kebenaran output dengan membandingkan hasil rendering purwarupa terhadap renderer referensi secara visual pada sekumpulan berkas SVG uji; kedua, pengujian performa untuk mengukur frame time dan skalabilitas terhadap penambahan core CPU, disertai analisis perbandingan terhadap renderer referensi yang telah ditetapkan.
