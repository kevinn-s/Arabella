...
Perluasan fungsionalitas berikut berpijak pada subset SVG yang saat ini didukung—terbatas pada elemen `g` dan `path` dengan pewarnaan solid (Subbab 4.2)—dan seluruhnya merupakan *pengembangan lanjutan* yang belum diwujudkan pada implementasi saat ini.
...
Kelima usulan ini menyediakan peta jalan penyempurnaan fungsionalitas yang berpijak pada subset SVG terdokumentasi (Subbab 4.2), di atas fondasi arsitektur *pipeline* hibrida *non-compute* yang telah terbukti berfungsi.

### 5.2.3 Saran Evaluasi Lanjutan

Evaluasi lanjutan yang utama adalah pelaksanaan *benchmark* kuantitatif langsung (*head-to-head*) antara Arabella dan renderer pembanding, yakni Skia, Cairo, dan Vello. Perbandingan pada penelitian ini masih kualitatif (Subbab 4.3.3), sedangkan data pada Subbab 4.3.2 hanya mengukur performa internal Arabella berupa dekomposisi waktu CPU dan GPU per *frame*, bukan perbandingan tatap muka. Posisi performa relatif antar-renderer karenanya belum dapat ditetapkan.

Karena seluruh angka performa Arabella terikat pada konfigurasi mesin uji tertentu (Tabel 4.1), *benchmark* lanjutan hanya sahih apabila dijalankan pada kondisi yang dikendalikan secara ketat dan seragam:
