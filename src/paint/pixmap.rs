...
pub struct Rgba8Pod(pub [u8; 4]);

impl From<peniko::color::Rgba8> for Rgba8Pod {
    fn from(c: peniko::color::Rgba8) -> Self {
        Rgba8Pod([c.r, c.g, c.b, c.a])
    }
}
...
impl Pixmap {
    ...
    pub fn into_png(self) -> Result<Vec<u8>, png::EncodingError> {
        ...
        let pixels: Vec<Rgba8Pod> = self.take_unpremultiplied()
    .into_iter()
    .map(Rgba8Pod::from)
    .collect();
